...
`journal.techne.coop` — a progressive web application for the **Open World Participation Framework** of RegenHub, LCA (Techne) — the same Common Information System that serves [techne.coop/intranet](https://techne.coop/intranet/), presented through a distinct, optimized UI/UX. Served by GitHub Pages from `main`.

**Status: public beta in preparation. Nothing here is adopted, and no schema change is made from this repository.**

## One repository, since 2026-09-02

This repository is the home of the Open World shell and the Daybook. The work began in [`Techne-Co-op/open-world`](https://github.com/Techne-Co-op/open-world) (its PR #4 was the last merged there) and was carried here whole on 2026-09-02 at the steward's direction, so that one PWA is served from one repository at one host. `app.techne.coop`, the host open-world's Pages configuration named, was never given a DNS record and will not be; `journal.techne.coop` is the address.

The first journal is preserved at [`/v1/`](/v1/) (see the end of this file).
...
It was moved rather than deleted (journal PR #1, 2026-08-31). Its internal links were rewritten to follow the move and nothing in it was otherwise edited. **Its Supabase wiring points at the `Techne Daybook` project, which is currently paused**, so its live surfaces do not presently reach a backend.
