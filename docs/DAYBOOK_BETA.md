...
**Honest seams.** The classifier is a word-pattern stand-in for the live model call. Persistence in the prototype is in-memory; the served build (`/daybook/`) reads and appends through one seam against the CIS `beta_` relations when signed in at the shell and the relation answers (D-06), and stages into `beta_staging` under the member's own identity (D-07). Signed out, the served route is a signpost and not a reading surface (D-07, after SHELL-A1); the in-memory demonstration answers only to `?seed`. A service worker (shell-only caching) is not present in the prototype — the `open-world` repository already ships one that caches the shell and never the record, and the build adopts that one rather than writing a second. Peer rereading is described but not built. §-addresses are routes in the served build (`#/§12`) and the frank copies the link; the prototype still copies the bare address. And the register's own unresolved question passes through unanswered: *what an append-only record owes a person who leaves* — the schema's only contribution is keeping it answerable in one place, with the decision before the members.
